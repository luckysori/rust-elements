    ...
    fn deserialize<D: Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        impl<'de> Visitor<'de> for CommitVisitor {
            ...
            fn visit_seq<A: SeqAccess<'de>>(self, mut access: A) -> Result<Self::Value, A::Error> {
                ...
                match prefix {
                    Some(0) => return Ok(Value::Null),
                    Some(1) => {
                        match access.next_element()? {
                            Some(x) => return Ok(Value::Explicit(u64::swap_bytes(x))),
                            None => return Err(A::Error::custom("missing explicit value")),
                        }
                    }
                    Some(2) => {
                        match access.next_element()? {
                            Some(x) => return Ok(Value::Confidential(x)),
                            None => return Err(A::Error::custom("missing pedersen commitment")),
                        }
                    }
                    _ => {
                        return Err(A::Error::custom("wrong or missing prefix"));
                    }
                };
            }
        }
    ...
    fn deserialize<D: Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        impl<'de> Visitor<'de> for CommitVisitor {
            ...
            fn visit_seq<A: SeqAccess<'de>>(self, mut access: A) -> Result<Asset, A::Error> {
                let prefix = access.next_element()?;
                match prefix {
                    Some(0) => return Ok(Asset::Null),
                    Some(1) => {
                        match access.next_element()? {
                            Some(x) => return Ok(Asset::Explicit(x)),
                            None => return Err(A::Error::custom("missing explicit asset")),
                        }
                    }
                    Some(2) => {
                        match access.next_element()? {
                            Some(x) => return Ok(Asset::Confidential(x)),
                            None => return Err(A::Error::custom("missing generator")),
                        }
                    }
                    _ => {
                        return Err(A::Error::custom("wrong or missing prefix"));
    ...
    fn deserialize<D: Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        impl<'de> Visitor<'de> for CommitVisitor {
            ...
            fn visit_seq<A: SeqAccess<'de>>(self, mut access: A) -> Result<Nonce, A::Error> {
                let prefix = access.next_element()?;
                match prefix {
                    Some(0) => return Ok(Nonce::Null),
                    Some(1) => {
                        match access.next_element()? {
                            Some(x) => return Ok(Nonce::Explicit(x)),
                            None => return Err(A::Error::custom("missing explicit nonce")),
                        }
                    }
                    Some(2) => {
                        match access.next_element()? {
                            Some(x) => return Ok(Nonce::Confidential(x)),
                            None => return Err(A::Error::custom("missing nonce")),
                        }
                    }
                    _ => {
                        return Err(A::Error::custom("wrong or missing prefix"));
